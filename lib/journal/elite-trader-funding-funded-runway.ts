...
import {
  getEliteTraderFundingFundedPayoutRowForAccount,
...
export type EliteTraderFundingFundedRunway = ApexFundedRunway;

export const ELITE_TRADER_FUNDING_FUNDED_PAYOUT_DASHBOARD_REMINDER =
  "Eligibility is subject to Elite Trader Funding rules on your dashboard.";
...
/**
 * Elite Trader Funding funded (CSV) : buffer, surplus, mini, paliers max 1er → 4e+ (indicatif).
 */
export function tryBuildEliteTraderFundingFundedRunway(
  state: JournalDataV1,
  account: JournalAccount,
  p: { startCents: number; currentCents: number }
): EliteTraderFundingFundedRunway | null {
  const csv = getEliteTraderFundingFundedPayoutRowForAccount(account);
  if (!csv) return null;
  ...
  return {
    ...
    etfFundedSimplePayoutUi: true,
    etfFundedSimplePayoutMinBalanceCents: tBufferEnd,
  };
}
