...
type Props = {
  ...
  tradeCount: number;
  onClose: () => void;
  /** Called with the day being deleted (avoids stale parent state). */
  onConfirm: (date: string) => void;
};

export function DeleteDayTradesModal({ open, date, tradeCount, onClose, onConfirm }: Props) {
  const canShow = open && date != null && tradeCount > 0;
  ...
  if (!mounted || !date || tradeCount === 0) return null;
  ...
  return (
    <div className="fixed inset-0 z-[170] flex items-center justify-center p-4">
      ...
      <div
        role="alertdialog"
        aria-modal="true"
        aria-labelledby="delete-day-trades-title"
        aria-describedby="delete-day-trades-desc"
      ...
      >
        ...
        <div className="px-5 py-5">
          <p
            id="delete-day-trades-desc"
            className="text-[14px] leading-relaxed text-zinc-400"
          >
            {tradeCount === 1
              ? "Delete 1 trade row"
              : `Delete ${tradeCount} trade rows`}{" "}
            for <span className="text-white/80">{date}</span> matching your current filters? This cannot be undone.
          </p>
          <div className="mt-6 flex flex-wrap justify-end gap-2">
            ...
            >
              {tradeCount === 1 ? "Delete row" : `Delete ${tradeCount} rows`}
            </button>
          </div>
        </div>
      </div>
