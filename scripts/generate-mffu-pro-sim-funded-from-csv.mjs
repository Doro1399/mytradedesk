...
const outPath = path.join(root, "lib", "journal", "mffu-pro-sim-funded-csv.generated.ts");
...
for (const rec of records) {
  ...
  if (program !== "My Funded Futures Pro") continue;
  rows.push(trimmed);
}

if (rows.length === 0) {
  console.error("No My Funded Futures Pro rows found in", csvPath);
  process.exit(1);
}

/** Funded block indices — `My Funded Futures Rules.csv` (Pro rows). */
function rowPro(line) {
  const f = parseCsvLine(line);
  const sizeRaw = (f[5] ?? "").trim().toLowerCase().replace(/\s+/g, "");
  const bufferUsd = parseMoneyUsd(f[27]);
  const fundedCalendarDaysFromFirstTrade = Number.parseInt(String(f[29] ?? "").replace(/\s/g, ""), 10);
  ...
  if (
    (sizeRaw !== "50k" && sizeRaw !== "100k" && sizeRaw !== "150k") ||
    !Number.isFinite(bufferUsd) ||
    bufferUsd <= 0 ||
    !Number.isFinite(fundedCalendarDaysFromFirstTrade) ||
    fundedCalendarDaysFromFirstTrade <= 0 ||
    !Number.isFinite(payoutMiniUsd) ||
    !Number.isFinite(payoutMaxUsd) ||
    !profitSplitLabel
  ) {
    console.error("Bad Pro funded parse", sizeRaw, f);
    process.exit(1);
  }
  ...
  return {
    size: sizeRaw,
    bufferUsd,
    fundedCalendarDaysFromFirstTrade,
    payoutMiniUsd,
    payoutMaxUsd,
    profitSplitLabel,
    notes,
...
for (const sz of order) {
  if (!bySize[sz]) {
    console.error("Missing Pro size", sz);
    process.exit(1);
  }
}
...
out += 'export type MffuProSimFundedCsvSize = "50k" | "100k" | "150k";\n\n';
out += "export type MffuProSimFundedCsvRow = {\n";
out += "  bufferUsd: number;\n";
out += "  /** Funded « Min trading days » — jours civils requis depuis le 1er trade Sim funded (CSV Pro = 14). */\n";
out += "  fundedCalendarDaysFromFirstTrade: number;\n";
out += "  payoutMiniUsd: number;\n";
out += "  payoutMaxUsd: number;\n";
out += "  profitSplitLabel: string;\n";
out += "  notes: string;\n";
out += "};\n\n";
out += "export const MFFU_PRO_SIM_FUNDED_FROM_CSV: Record<MffuProSimFundedCsvSize, MffuProSimFundedCsvRow> = {\n";

for (const sz of order) {
  const r = bySize[sz];
  out += `  "${sz}": {\n`;
  out += `    bufferUsd: ${r.bufferUsd},\n`;
  out += `    fundedCalendarDaysFromFirstTrade: ${r.fundedCalendarDaysFromFirstTrade},\n`;
